    ...
    switch (cmd) {
      case 'help':
        segmentOutput = [
          ...
          '  Output:       lines · prune 10-500 · prune stream',
          '                scroll atas/bawah = geser jendela 50 baris',
          '                (riwayat penuh di cache, DOM max 50 baris)',
          ...
          '',
          '  Port / server (Windows):',
          '    ports                       — lihat semua port LISTENING + PID',
          '    ports 4201                  — cek port tertentu',
          '    killport 4201               — hentikan port + worker node terkait',
          '    killnode                    — hentikan semua node/npm di folder ini',
          '    killnode all                — hentikan semua node.exe (hati-hati)',
          '    kp / kn                     — alias singkat',
          '',
          '  Server dev (folder ini):',
          '    running / dev / rn          — cek npm/node/server yang masih jalan',
          '    Saat npm jalan: output live di layar saja (tidak disimpan ke disk)',
          '    Ctrl+C                      — hentikan proses di tab ini',
          '    taskkill /IM node.exe /F    — matikan SEMUA node (hati-hati)',
          '    netstat -ano | findstr :4200 — cek PID manual',
          '',
          '  Git / push ke GitHub:',
  ...
  try {
    if (isStreamCommand(trimmed, state.aliases)) {
      ...
      return {
        ...
        delegatedToCmd: data.delegatedToCmd === true,
        output: [],
        newState: {
          ...state,
          cwd: data.cwd ?? state.cwd,
